...
if (!exists('graddist_collect')) {
  dist_outDir <- 'output/validation_TDtopology_distance_individual'
  ...
  export(graddist_collect, file = 'output/validation_TDtopology_distance_collection.csv')
}
...
## 3D plotly ------------------------------------------------------------------
dat_collect$network <- factor(dat_collect$network,  
                              levels = c("visual", "somMot",'dorsalAttn',"salience","control","limbic","DMN"))

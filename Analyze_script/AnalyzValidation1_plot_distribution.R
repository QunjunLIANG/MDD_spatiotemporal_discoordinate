...
info_dat <- import('MDD_validation_dataset.xlsx')

dat_file_grad <- list.files('inputs/validation_gradient_inputs/', pattern = 'sub*', full.names = T)

dat_file_tde_topo <- list.files('inputs/validation_TDE_Gradient/', pattern = 'sub.*align-procrustes.csv', full.names = T)
...
## group difference
dat_tmp <- dat_all_long %>% group_by(parcel) %>% summarize_each(funs = funs(mean))

p_td_topo_distribution <- ggplot(data = dat_tmp, aes(x = td_topo, y =..density..)) + 
  geom_histogram(data = dat_tmp, 
                 bins = 50, 
                 aes(x = td_topo, y =..density..), 
                 fill = mdd_color,
                 color="white", alpha = 0.8)+
  geom_density(aes(x = td_topo, y =-..density..),
               fill = mdd_color,
               color = 'white', alpha = 0.6, size = 1) +
  xlab('TD-based gradient score') + ylab('Density') + 
...
## group difference
p_grad_distribution <- ggplot(data = dat_tmp, aes(x = gradient, y =..density..)) + 
  geom_histogram(data = dat_tmp, 
                 bins = 50, 
  ...
  geom_histogram(data = dat_tmp, 
                 ...
                 fill = mdd_color,
                 color="white", alpha = 0.8) +
  geom_density(aes(x = gradient, y =-..density..),
               fill = mdd_color,
               color = 'white', alpha = 0.6, size = 1) +
...
dat_tmp <- dat_all_long[,c(-2, -6,-7)] %>% 
  melt(id.vars = c('participant_id','parcel', 'network'), 
       variable.name = "measure", value.name = 'value') %>%
  group_by(parcel,network, measure) %>%
  summarize_each(funs = funs(mean))

facet_labels <- c(gradient = "Functional gradient", td_topo="TD-based gradient")
p_net_distribution <- ggplot(data = dat_tmp, aes(y=network, x=value)) +
...
dat_tmp <- dat_all_long %>% group_by(parcel,network) %>%
  summarize_each(funs = funs(mean))

ggscatterstats(dat_tmp,
                       x = gradient,
                       y = td_topo,
                       bf.message = F,
...
# combine the plots -------------------------------------------------------
p_distribution <- (p_td_topo_distribution + p_grad_distribution + p_corplot)/p_net_distribution
p_distribution + plot_annotation(tag_levels = c('a')) & 
  theme(plot.tag = element_text(size = 22, face = 'bold'))
ggsave('output/validation_Distribution_plots.png', width = 13, height = 10)
...
export(dat_all_long, file = file.path(outDir, 'validation_TD_topo_Gradient_collect_long.csv'))

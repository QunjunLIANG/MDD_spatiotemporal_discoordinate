...
sbj_file     = 'MDD_HC_match_data.tsv' # subject file in BIDS
...
# determine which ROI should be use
roi_select   = '/home/lqj/MDD_patient/R_project/tde_gradient_coupling_in_-mdd/output/Power2011ROI_clean_filtered.csv'

# export timeseries
outDir       = wkDir + 'derivatives/timeseries_for_gradient_Power264/'
FD_out_file  = outDir + 'xxxx_FD.csv'
TS_out_file  = outDir + 'xxxx_timeseries_power264.csv'
...
para_denosie  = "simple" # one of "simple", "scrubbing", "compcor", "ica_aroma"

# obtain parcellations and make it into a labeling masker
atlas_dataset = datasets.fetch_coords_power_2011()
atlas_map = atlas_dataset.rois

## filter the ROIs from 264 to 214
roiTable = pd.read_csv(roi_select)
roiSelect = roiTable['parcel_id']
roiSelect = np.array(roiSelect) -1

## transfer the rois into numpy array
coords_raw = np.vstack((atlas_map["x"],atlas_map["y"],atlas_map["z"])).T
coords = np.take(coords_raw, roiSelect, axis = 0)

...
%% Setup
% Set parameters
num_nodes      = 400;    % number of time series, also, number of ROIs. 4872 for voxel-wise.
wkDir          = '/home/lqj/MDD_patient/NIfTI_convert/derivatives/timeseries_for_EST/';
...
sbjNamePath    = '/home/lqj/MDD_patient/NIfTI_convert/';
subject_list = readtable([sbjNamePath 'ETS_participant_info.csv'], 'FileType', 'text', 'Delimiter', ',', 'TreatAsEmpty', 'NA', 'ReadVariableNames', true);
...
outFile_prefix = 'noGSR'; % add string to the output csv to identify the analysis
tr             = 2;
...
for s = useSubject
    ...
    BOLD = load([wkDir subj '_timeseries_schaefer400.csv']); % read in time series matrix
    % BOLD = BOLD.data;
